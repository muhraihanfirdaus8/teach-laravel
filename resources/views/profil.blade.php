...
<h1 style='color:#0B1F3A;margin-bottom:20px;'>
    ...
    <span style='font-size:14px;color:#666;font-weight:normal;'>({{ $mahasiswas->count() }} mahasiswa)</span>
</h1>
    ...
    <div style='background:white;border-radius:10px;padding:20px;margin-bottom:16px;border-left:4px solid #065A82;box-shadow:0 2px 6px rgba(0,0,0,.08);'>
        <div style='display:flex;justify-content:space-between;align-items:center;'>
            <div>
                ...
                <p style='color:#666;margin:4px 0;font-size:14px;'>{{ $mhs->nim }} • {{ $mhs->prodi }} {{ $mhs->angkatan }}</p>
            </div>
            <div style='text-align:right;'>
                <div style='font-size:22px;font-weight:bold;color:#065A82;'>{{ $mhs->ipk }}</div>
                @if($mhs->ipk >= 3.75)
        ...
        @endif
        <a href="{{ route('mahasiswa.show', $mhs->id) }}" style='color:#065A82;font-size:13px;text-decoration:none;'>Lihat Detail →</a>
    </div>
@endforeach
@endsection