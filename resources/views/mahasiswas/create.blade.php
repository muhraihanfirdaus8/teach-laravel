    ...
    <form action="{{ route('mahasiswas.store') }}" method='POST' style='background:white;padding:24px;border-radius:10px;border:1px solid #ddd;'> 
        ...
        <div style='display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;'> 
            ...
            <div>
                ...
                <input type='number' name='angkatan' value="{{ old('angkatan', 2022) }}" style='width:100%;padding:10px;border:1px solid #ddd;border-radius:6px;box-sizing:border-box;'> 
            </div>
        </div>
        ...
        <div style='display:flex;gap:10px;'> 
            <button type='submit' style='background:#065A82;color:white;padding:10px 24px;border:none;border-radius:6px;cursor:pointer;font-size:15px;'>Simpan Mahasiswa</button> 
            <a href="{{ route('mahasiswas.index') }}" style='padding:10px 20px;border:1px solid #ddd;border-radius:6px;text-decoration:none;color:#666;'>Batal</a> 
        </div>
    </form>
